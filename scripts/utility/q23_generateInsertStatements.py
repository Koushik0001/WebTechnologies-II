...
def insert_space(string):
    # Use a regular expression to match any upper case letter followed by a lower case letter
    pattern = re.compile(r'(?<=\w)([A-Z][a-z])')
    
    # Replace the match with a space and the original match
    return re.sub(pattern, r' \1', string)


statesFile = open('../../Database/states.sql', 'w')
districtsFile = open('../../Database/districts.sql', 'w')

i=0
j = 0

for key in indianStates:
    upper_letters = ""
    for char in key:
        if char.isupper():
            upper_letters += char
    state = insert_space(key)
    stateID = "{0}{1}".format(upper_letters,i)
    print("INSERT INTO states VALUES('{0}','{1}');".format(stateID, state), file = statesFile)
    for dist in indianStates[key]:
        upper_letters = ""
        for char in dist:
            if char.isupper():
                upper_letters += char
        print("INSERT INTO districts VALUES('{0}{1}','{2}','{3}','{4} is a district of {5}');".format(upper_letters, j,dist, stateID, dist, state), file=districtsFile)
        j=j+1
    i=i+1
